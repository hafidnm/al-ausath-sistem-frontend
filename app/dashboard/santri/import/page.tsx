...
const parseCsvText = (content: string): PreviewRow[] => {
  ...
  return rows.map((line) => {
    ...
    return {
      ...
      nama_lengkap_santri: rowObj.nama_lengkap_santri || "",
...
export default function SantriImportPage() {
  const { toast } = useToast()
  const [file, setFile] = useState<File | null>(null)
  const [previewRows, setPreviewRows] = useState<PreviewRow[]>([])
  const [isPreviewAvailable, setIsPreviewAvailable] = useState(true)
  const [isLoading, setIsLoading] = useState(false)
  const [importSummary, setImportSummary] = useState<DataSantriImportSummary | null>(null)
  ...
  const handleDownloadTemplate = () => {
    const run = async () => {
      try {
        const blob = await dataSantriService.downloadImportTemplate()
        downloadBlob(blob, "template-import-santri.csv")
      } catch (error) {
        const err = error as { response?: { data?: { message?: string } } }
        toast({
          title: "Gagal",
          description: err.response?.data?.message || "Gagal mengunduh template impor santri.",
          variant: "destructive",
        })
      }
    }
  ...
  const handleProcessImport = () => {
    const run = async () => {
      ...
      setIsLoading(true)
      try {
        const result = await dataSantriService.importFile(file)
        ...
        toast({
          title: "Gagal",
          description: err.response?.data?.message || "Gagal memproses impor data santri.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <h1 className="text-3xl font-semibold tracking-tight text-foreground">IMPOR DATA SANTRI</h1>
