...
import { DataPetugasImportSummary, dataPetugasService } from "@/lib/services/petugas.service"
...
export default function GuruImportPage() {
  ...
  const [importSummary, setImportSummary] = useState<DataPetugasImportSummary | null>(null)
  ...
  const handleDownloadTemplate = () => {
    const run = async () => {
      try {
        const blob = await dataPetugasService.downloadImportTemplate()
        downloadBlob(blob, "template-import-petugas.csv")
      } catch (error) {
        const err = error as { response?: { data?: { message?: string } } }
        toast({
          title: "Gagal",
          description: err.response?.data?.message || "Gagal mengunduh template impor petugas.",
          variant: "destructive",
        })
      }
    }
  ...
  const handleProcessImport = () => {
    const run = async () => {
      ...
      setIsLoading(true)
      try {
        const result = await dataPetugasService.importFile(file)
        ...
        toast({
          title: "Gagal",
          description: err.response?.data?.message || "Gagal memproses impor data petugas.",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
      ...
      <Card>
        <CardContent className="space-y-8 p-6">
          <div className="space-y-4">
            <p className="text-2xl text-foreground">Silahkan unduh templat excel impor data petugas berikut:</p>
            ...
            <TableBody>
              {!isPreviewAvailable ? (
                ...
                previewRows.map((row, index) => (
                  <TableRow key={`${row.nomor_induk}-${index}`}>
