...
import {
  DataJadwalPembelajaranImportSummary,
...
export default function JadwalPembelajaranImportPage() {
  const { toast } = useToast()
  const [file, setFile] = useState<File | null>(null)
  const [previewRows, setPreviewRows] = useState<PreviewRow[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [importSummary, setImportSummary] = useState<DataJadwalPembelajaranImportSummary | null>(null)
  ...
  const handleDownloadTemplate = () => {
    const run = async () => {
      try {
        const blob = await dataJadwalPembelajaranService.downloadImportTemplate()
        downloadBlob(blob, "template-import-jadwal-pembelajaran.csv")
      } catch (error) {
        toast({
          title: "Gagal",
          description: getErrorMessage(error, "Gagal mengunduh template impor data jadwal pembelajaran."),
          variant: "destructive",
        })
      }
    }
  ...
  const handleProcessImport = () => {
    const run = async () => {
      ...
      setIsLoading(true)
      try {
        const result = await dataJadwalPembelajaranService.importFile(file)
        ...
        toast({
          title: "Gagal",
          description: getErrorMessage(error, "Gagal memproses impor data jadwal pembelajaran."),
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <h1 className="text-3xl font-semibold tracking-tight text-foreground">IMPOR DATA JADWAL PEMBELAJARAN</h1>
        <Link href="/dashboard/jadwal-pembelajaran">
      ...
      <Card>
        <CardContent className="space-y-8 p-6">
          <div className="space-y-4">
            <p className="text-2xl text-foreground">Silakan unduh template CSV impor data jadwal pembelajaran:</p>
