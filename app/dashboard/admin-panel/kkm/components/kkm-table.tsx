...
import { KkmItem } from "@/lib/services/kkm.service"
import { normalizeUnitLabel } from "../utils/helpers"

interface KkmTableProps {
  items: KkmItem[]
  isLoading?: boolean
  error?: string
...
export function KkmTable({ items, isLoading = false, error, onEdit, onDelete }: KkmTableProps) {
  ...
  return (
    <>
      <Card className="border-border/50">
        <CardHeader>
          <CardTitle className="text-lg text-foreground">Daftar KKM</CardTitle>
          <CardDescription>KKM per mapel, semester, tahun ajaran, dan unit</CardDescription>
        </CardHeader>
              ...
              <TableBody>
                ...
                {!isLoading && !error && items.map((item, index) => {
                  ...
                  const rowKey = hasValidId
                    ? `kkm-${item.id}`
                    : `kkm-fallback-${item.kode_mapel}-${item.tahun_ajaran}-${item.semester}-${index}`
      ...
      <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus data KKM?</AlertDialogTitle>
            <AlertDialogDescription>
              Data KKM yang dihapus tidak dapat dikembalikan.
            </AlertDialogDescription>
          </AlertDialogHeader>
